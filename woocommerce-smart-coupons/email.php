...
<?php
	$email_heading="You have received a Gift Certificate for Hannah Sowd Skin Care!";
...
?>
<p><?php echo sprintf(__("Your Gift Certificate worth ", WC_Smart_Coupons::$text_domain), $blogname); ?>
<b><?php if ( ! empty( $coupon_data['coupon_amount'] ) && $coupon->amount != 0 ) {
						echo $coupon_data['coupon_amount'];
						if ( $coupon->free_shipping == "yes" ) {
							echo __( ' &amp; ', WC_Smart_Coupons::$text_domain );
						}
					} ?></b>
<?php echo sprintf(__(" may be redeemed for services at Hannah Sowd Skin Care or applied to your online purchase at www.hannahsowd.com", WC_Smart_Coupons::$text_domain), $blogname); ?></p>

<div style="margin: 10px 0; text-align: center;" title="<?php echo __( 'Click to visit store. This Gift Certificate will be applied automatically.', WC_Smart_Coupons::$text_domain ); ?>">
			...
			<?php
				echo '<div class="coupon-content blue dashed small">
					...
					if ( ! empty( $coupon_data['coupon_amount'] ) && $coupon->amount != 0 ) {
						echo $coupon_data['coupon_amount'] . ' Gift Certificate';
						if ( $coupon->free_shipping == "yes" ) {
							echo __( ' &amp; ', WC_Smart_Coupons::$text_domain );
						}
					}
...
<center><p><a href="<?php echo $url; ?>"><?php echo sprintf(__("Visit Store",WC_Smart_Coupons::$text_domain) ); ?></a></p></center>
<center><p><a href="http://hannahsowd.genbook.com"><?php echo sprintf(__("Book Appointment",WC_Smart_Coupons::$text_domain) ); ?></a></p></center>

<?php if ( !empty( $from ) ) { ?>
	<p><?php echo __( 'You got this gift certificate', WC_Smart_Coupons::$text_domain ) . ' ' . $from . $sender; ?></p>
<?php } ?>
